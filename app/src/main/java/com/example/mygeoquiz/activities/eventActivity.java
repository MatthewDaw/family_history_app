...
public class eventActivity  extends AppCompatActivity implements OnMapReadyCallback {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        familyData = MainActivity.familyData;
        settings = MainActivity.settings;

        Intent intent = getIntent();

        String selectedEventID = intent.getStringExtra("com/example/sharedcodemodule");

        selectedEvent = familyData.eventMap.get(selectedEventID);

        selectedPerson = familyData.personMap.get(selectedEvent.getPersonID());

        tv1 = (TextView) findViewById(R.id.mapTextViewName);
        tv2 = (TextView)  findViewById(R.id.mapTextViewSub);

        imgV = (ImageView) findViewById(R.id.eventIcon);
        footBox = (RelativeLayout) findViewById(R.id.InnerRelativeLayout);
        ...
        footBox.setOnClickListener(new View.OnClickListener(){
            ...
            public void onClick(View v){
                if(selectedEvent != null && selectedPerson != null ){
                    Intent resultIntent = new Intent();
                    resultIntent.putExtra("result", selectedPerson.getPersonID());
                    setResult(RESULT_OK, resultIntent);
                    finish();
                }
            }
    ...
    public void placeStartupMarkers(){
        for(int i = 0; i < familyData.eventResponseG.getEvents().length; i++){
            ...
            if(checkEvent(familyData.eventMap.get(eventID))) {
